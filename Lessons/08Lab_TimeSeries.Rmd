...
sum(is.na(Temp_BR_processed))
sum(is.na(Rainfall_BR_processed))
...
temp_Decomposed <- stl(temp_BR_ts, s.window = "periodic")
plot(temp_Decomposed)

rainfall_Decomposed <- stl(Rainfall_BR_ts, s.window = "periodic")
...
temp_df <- as.data.frame(temp_Decomposed$time.series[,1:3])

temp_df <- mutate(temp_df,
        Observed = Temp_BR_processed$Temperature_C,     
        Date = Temp_BR_processed$Date)

rainfall_df <- as.data.frame(rainfall_Decomposed$time.series[,1:3])

rainfall_df <- mutate(rainfall_df,
