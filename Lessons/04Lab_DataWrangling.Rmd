...
# OR
# NTL.phys.data.1999 <- NTL.phys.data %>% filter(year(sampledate)==1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.90_99 <- filter(NTL.phys.data, year4 %in% c(1990:1999), lakeid == "T")
# OR
# NTL.phys.data.90_99 <- filter(NTL.phys.data, (year4 %in% c(1990:1999)) & lakeid == "T")
```
...
> Answer: Time consuming to observe the row numbers manually. Also the row numbers are not fixed, can be rearranged
...
NTL.phys.data.processed <- NTL.phys.data %>%
...
NTL.phys.data.exercise3 <- NTL.phys.data %>%
...
Exercise4 <- NTL.phys.data.exercise3 %>% 
  filter(depth == 0) %>% 
...
Exercise5 <- pivot_longer(NTL.phys.data, 
                          cols= c('irradianceWater', 'irradianceDeck'),
                          names_to = 'irradiance')

#Exercise 6: Spread temperatureC into more than one column based on the depth.

new <- cbind.data.frame(temperature_C = NTL.phys.data$temperature_C,
                        depth = NTL.phys.data$depth)

Exercise6 <- pivot_wider(new, names_from = depth,
