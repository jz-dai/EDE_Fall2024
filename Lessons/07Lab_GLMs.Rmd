...
# density plot
O3_density <- ggplot(EPAair,aes(x=Ozone, fill = as.factor(Year))) + 
  geom_density(alpha=0.25) +
  scale_fill_manual(values = c('green','orange'))
  labs(title="Density of Ozone AQI value by Year",
       x='Ozone AQI value')
...
# boxplot
O3_box <- ggplot(EPAair,aes(x=Year, y=Ozone)) + 
  geom_boxplot() +
  stat_summary(geom='text',
               ...
               size=4,
               label=c("b","a"))+
  labs(title="Boxplot of Ozone AQI value by Year",
       x='Year')

print(O3_box)

# violin
O3_violin <- ggplot(EPAair,aes(x=Year, y=Ozone)) + 
  geom_violin() +
  stat_summary(geom='text',
               fun=max,
               vjust=1,
               size=4,
               label=c("b","a"))+
  labs(title="Violin plot of Ozone AQI value by Year",
       x='Year')
...
PM25_by_O3 <- lm(EPAair$PM2.5~ EPAair$Ozone)
summary(PM25_by_O3)
...
scatter_O3 <- 
  ggplot(EPAair, aes(y = PM2.5, x = Ozone)) +
  geom_point()+
  geom_smooth(method = "lm", col = "red")+
  labs(title='Scatterplot of PM2.5 by Ozone',
...
EPAPM2.5 <- 
  ...
  na.omit()
PM2.5_Plot <- cor(EPAPM2.5)
corrplot(PM2.5_Plot, method = "circle")
corrplot.mixed(PM2.5_Plot, upper = "circle")
...
PM25_global <- lm(EPAPM2.5$PM2.5~ EPAPM2.5$Ozone + 
                    EPAPM2.5$mean_Lat +EPAPM2.5$mean_Lng + EPAPM2.5$Month+
                    EPAPM2.5$Year)
summary(PM25_global)
