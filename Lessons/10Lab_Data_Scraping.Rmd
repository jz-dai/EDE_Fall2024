...
the_states <- 'td:nth-child(1)'
the_price <- 'td:nth-child(2)'
the_capacity <- 'td:nth-child(3)'
net_generation <- 'td:nth-child(4)'
total_retail <- 'td:nth-child(5)'
  
#3 Construct a dataframe from the values
states <- the_website %>% html_nodes(the_states) %>% html_text()
price <- the_website %>% html_nodes(the_price) %>% html_text()
capacity <- the_website %>% html_nodes(the_capacity) %>% html_text()
generation <- the_website %>% html_nodes(net_generation) %>% html_text()
retail <- the_website %>% html_nodes(total_retail) %>% html_text()

energy_2023 <- data.frame('State.Name' = states, 'Average.retail.price.(cents/kWh)' = as.numeric(price), 'Net.summer.capacity.(MW)' = as.numeric(gsub(",","",capacity)), 'Net.Generation.(MWh)' = as.numeric(gsub(",","",generation)), "Total.retail.sales.(MWh)" = as.numeric(gsub(",","",retail)))
```
